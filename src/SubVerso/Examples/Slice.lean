...
private def sliceCommand (range : Compat.Syntax.Range) : Parsec SliceCommand := do
  ws
  skipString "!!"
  ws
  beginCmd <|> endCmd
...
-/
private def syntaxIn (stx : Syntax) (rng : Compat.Syntax.Range) : Bool :=
  if let some sr := stx.getRange? then
    rng.includes sr
  else
    false

private partial def removeRanges (env : Environment) (rngs : Array Compat.Syntax.Range) (stx : Syntax) : Syntax :=
  match stx with
...
private def getSlices (slices : Array SliceCommand) : Except String (HashMap String (Array Compat.Syntax.Range)) := do
