...
private def isAnchorLike (str : String) : Bool := Id.run do
  let mut iter := str.iter
...
private def isStateLike (str : String) : Bool := Id.run do
  let mut iter := str.iter
  while iter.hasNext do
    iter := iter.find fun c => c == 'A' || c == 'P'
